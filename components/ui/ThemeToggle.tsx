...
export const createAnimation = (
  ...
  const maxMaskSize = isMobile ? "200vmax" : "350vmax";
  const blurStart = isMobile ? "2px" : "8px";
  const blurMid = isMobile ? "1px" : "4px";
  const duration = isMobile ? "0.8s" : "1s";

  // ===== Circle-blur / GIF / Circle variants =====
  if (variant === "circle-blur" || variant === "circle") {
    return {
      name: `${variant}-${start}`,
      css: `
        ::view-transition-group(root) {
          animation-timing-function: var(--expo-out);
          will-change: transform, mask-size, filter, clip-path;
        }

        ::view-transition-new(root) {
          ${variant === "circle-blur" ? `mask: url('${svg}') ${start.replace("-", " ")} / 0 no-repeat; mask-origin: content-box;` : ""}
          transform-origin: ${transformOrigin};
          animation: scale-${start} ${duration};
          ${blur ? `filter: blur(${blurStart});` : ""}
        }
        ...
        @keyframes scale-${start} {
          from { ${variant === "circle-blur" ? "mask-size: 0;" : "clip-path: circle(0% at 50% 50%);"} ${blur ? `filter: blur(${blurStart});` : ""} }
          50% { ${blur ? `filter: blur(${blurMid});` : ""} }
          to { ${variant === "circle-blur" ? `mask-size: ${maxMaskSize};` : "clip-path: circle(100% at 50% 50%);"} ${blur ? "filter: blur(0px);" : ""} }
        }
      `,
    };
  }
  ...
  if (variant === "gif" && url) {
    return {
      name: `${variant}-${start}`,
      css: `
        ::view-transition-group(root) { animation-timing-function: var(--expo-in); will-change: mask-size, transform; }
        ::view-transition-new(root) { mask: url('${url}') center / 0 no-repeat; animation: gifScale ${duration}; }
        ::view-transition-old(root), .dark::view-transition-old(root) { animation: gifScale ${duration}; z-index: -1; }
        @keyframes gifScale { from { mask-size: 0; } to { mask-size: ${maxMaskSize}; } }
      `,
    };
  }

  // ===== Rectangle / Polygon / Other variants =====
  return {
    name: `${variant}-${start}`,
    css: `
      ::view-transition-group(root) { animation-timing-function: var(--expo-out); will-change: clip-path, transform, filter; }
    `,
  };
};
